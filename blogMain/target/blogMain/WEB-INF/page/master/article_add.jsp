...
<div class="container-fluid">
    <div class="row">
        ...
        <div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">
            <%--4行--%>
            <%--标题--%>
            <h1 class="page-header">ARTICLE-ADD</h1>
            <%--按钮--%>
            <form id="article_add_form">
                <div class="form-group">
                    <label for="title_add_input"><b>标题</b></label>
                    <input type="text" name="title" class="form-control" id="title_add_input" placeholder="title">
                    <span class="help-block"></span>
                </div>
                <div class="form-group">
                    <label for="author_add_input"><b>作者</b></label>
                ...
                <div class="form-group">
                    <label for="brief_add_text"><b>摘要</b></label>
                    <textarea class="form-control" name="brief" id="brief_add_text" placeholder="brief" rows="4"
                              cols="20"></textarea>
                </div>
                <div class="form-group">
                    <label><b>分类</b></label>
                ...
                <div class="form-group">
                    <label for="img_add_input"><b><a href="#"
                                                     onclick="return insertDescImg()">插入摘要右侧图片</a></b></label>
                    <input type="text" name="imgUrl" class="form-control" id="img_add_input" placeholder="img">
                    <span class="help-block"></span>
                </div>
                <button type="button" class="btn btn-primary" id="article_save_btn">发布</button>
                <button type="button" class="btn btn-primary" id="article_saveDraft_btn">存为草稿</button>
                <button type="button" class="btn btn-danger" id="article_cancel_btn" onclick="javascrtpt:window.location.href='${APP_PATH}/articlePage'">取消</button>
...
<script type="text/javascript">
    ...
    //1、页面加载完成以后，获取文章信息
    $(function () {
        getCategories("#category_add_select");
    })
    ...
    $("#title_add_input").change(function () {
        ...
        if (title == "") {
            ...
            $("#article_saveDraft_btn").attr("ajax-va", "error");
            return false;
        }
        else {
            show_validate_msg("#title_add_input", "success", "");
            $("#category_save_btn").attr("ajax-va", "success");
            $("#article_saveDraft_btn").attr("ajax-va", "success");
        }
    });
    ...
    $("#article_save_btn").click(function () {
        //1、拿到要校验的数据，正则表达式
        if ($("#title_add_input").val() == "") {
            $("#title_add_input").focus();
            show_validate_msg("#title_add_input", "error", "请输入标题");
            return false;
        }
        ...
        $.ajax({
            ...
            data: $("#article_add_form").serialize()
                + '&' + $.param({
                    // content: $("#divDemo").prop("outerHTML"),    //包括<div id=...>自身
                    content: $("#divDemo .w-e-text").html(),        //不加.w-e-text会把冗余<div>添加进来
                    postTime: new Date(),
                    pageviewNum: 0,
                    favorNum: 0
                }),
    ...
    //存为草稿
    $("#article_saveDraft_btn").click(function () {
        //1、拿到要校验的数据，正则表达式
        if ($("#title_add_input").val() == "") {
            $("#title_add_input").focus();
            show_validate_msg("#title_add_input", "error", "请输入标题");
        ...
        $("#category_add_select").val(2);       //类别设置为草稿，其实可以用isDraft的tag来设置为草稿。。
        //2、发送ajax请求保存文章
        $.ajax({
            url: "${APP_PATH}/article",
            type: "POST",
    ...
    function insertDescImg() {
        ...
        document.getElementById("img_add_input").value = strImg + document.getElementById("img_add_input").value;
        document.getElementById("img_add_input").focus();
        return false;
    }
